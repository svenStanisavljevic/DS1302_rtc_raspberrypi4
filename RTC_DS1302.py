...
class RTC_DS1302:
    parser = ConfigParser()
    parser.read('config')
    RTC_DS1302_SCLK = int(parser.get('pins', 'clk_pin'))
    RTC_DS1302_CE = int(parser.get('pins', 'rst_pin'))
    RTC_DS1302_IO = int(parser.get('pins', 'dat_pin'))

    #if clock is tracking 
    CLK_PERIOD = 0.001
...
# Read date and time data.
        ...
        Data = format(DateTime["Year"] + 2000, "04d") + "-" + format(DateTime["Month"], "02d") + "-" + format(DateTime["Day"], "02d")
